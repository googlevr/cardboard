...
public class QrCodeContentProcessor {
  ...
  private static final int HTTP_TIMEOUT_MS = 5 * 1000;
  ...
  /**
   * Attempts to converts a QR code detection result into device parameters (as a protobuf).
   *
   * @param barcode The detected QR code.
   * @param urlFactory Factory for creating URL instance for HTTP connection.
  ...
  private static QrCodeToParamsStatus getParamsFromQrCode(Barcode barcode, UrlFactory urlFactory) {
    ...
    // If needed, prefix free text results with a http prefix.
    if (barcode.valueFormat == Barcode.TEXT && uri.getScheme() == null) {
      uri = Uri.parse(HTTP_SCHEME_PREFIX + uri);
      if (uri == null) {
        return QrCodeToParamsStatus.error(QrCodeToParamsStatus.STATUS_UNEXPECTED_FORMAT);
      }
    }
  ...
  /**
   * Follow HTTP redirect until we reach a valid Cardboard device URI.
   *
   * <p>Network access is only used if the given URI is not already a cardboard device. Only HTTP
  ...
  private static Uri followCardboardParamRedirect(
    ...
    while (uri != null && !CardboardParamsUtils.isCardboardUri(uri)) {
      ...
      uri = resolveHttpRedirect(uri, urlFactory);
      numRedirects++;
    }
    return uri;
  }
  ...
  /**
   * Dereference an HTTP redirect without reading resource body.
   *
   * @param uri The initial URI.
   * @param urlFactory Factory for creating URL instance for HTTP connection.
   ...
   */
  @Nullable
  private static Uri resolveHttpRedirect(Uri uri, UrlFactory urlFactory) throws IOException {
    HttpURLConnection connection = urlFactory.openHttpConnection(uri);
    if (connection == null) {
      return null;
    }
